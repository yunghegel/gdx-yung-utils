...
public abstract class AbstractConsole extends VisTable implements Console, Disposable {
    public final Log log;
    protected CommandExecutor exec;
    protected boolean logToSystem= true;
    ...
    @Override public void log (String msg){
        LogLevel lvl;
        if (msg.startsWith("[INFO]")) lvl = LogLevel.INFO;
        else if (msg.startsWith("[ERROR]")) lvl = LogLevel.ERROR;
        else if (msg.startsWith("[SUCCESS]")) lvl = LogLevel.SUCCESS;
        else if (msg.startsWith("[DEBUG]")) lvl = LogLevel.DEBUG;
        else lvl = LogLevel.DEFAULT;

        //split the message into two parts, the first being first line, the second being the rest
        String[] split = msg.split("\n", 2);
        log.addEntry(split[0], lvl);
        if (split.length > 1) log.addEntry(split[1], LogLevel.DEFAULT);

//        log.addEntry(log.format(msg), LogLevel.DEFAULT);
    }

    @Override public void log (Throwable exception, LogLevel level) {
        log.addEntry(ConsoleUtils.exceptionToString(exception), level);
    }

    @Override
    public void log(LogRecord record) {
        log.addEntry(log.format(record), log.getLogLevel(record.getLevel()));
    }
